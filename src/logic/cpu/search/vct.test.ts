...
describe("hasVCT", () => {
  ...
  it("既に五連がある場合はVCT探索対象外（勝利済み）", () => {
    ...
    // 五連は既に勝利済みの状態であり、VCT探索の脅威手が見つからないためfalse
    expect(hasVCT(board, "black")).toBe(false);
  });
});
...
describe("石数閾値の動作", () => {
  it("14石未満ではVCTスキップ（活三があってもfalse）", () => {
    // 13石: VCTスキップ
    const board = createBoardWithStones([
      { row: 7, col: 5, color: "black" },
      { row: 7, col: 6, color: "black" },
    ...
    expect(countStones(board)).toBe(13);
    // stoneThreshold=14（デフォルト）でスキップ
    expect(hasVCT(board, "black", 14)).toBe(false);
    // stoneThreshold=0ではVCTが見つかる
    expect(hasVCT(board, "black", 0)).toBe(true);
  });

  it("14石以上ではVCT探索が実行される", () => {
    const board = createBoardWithStones([
      { row: 7, col: 5, color: "black" },
      { row: 7, col: 6, color: "black" },
      { row: 7, col: 7, color: "black" },
    ...
    expect(countStones(board)).toBe(14);
    expect(hasVCT(board, "black")).toBe(true);
  });
});
