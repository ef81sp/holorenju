...
**受け入れ棋譜（issue #6 由来, 確定）**:

```
H8 H7 F6 E5 F8 F7 E7 E8 G6 D8 G9 D6 G7 G8 J6
```

- **9手目（黒番）の最善が VCT 必勝**。第一階層分岐 5本に加え、side 分岐内に更なる分岐（深い分岐）が多数存在する。
- 確定方法: vct.zig に一時計測（buildBranches が捨てる非主筋 entry の `child_branch_count > 0` をカウント）を入れ、クローズ issue の候補棋譜群を直列スキャン。本棋譜の prefix=8（黒番）で `branch_count=5` かつ深い分岐ドロップを確認（他に #11 prefix=18、#5 prefix=6 でも再現）。計測コードは確認後に撤去済み。
- **受け入れ条件**: 修正後、9手目の「最善」タブで side 分岐（代替手）を選択すると、その continuation 内にさらなる分岐タブが現れること（現状は第一階層しか出ない）。

予備の再現棋譜: `#11` (`H8 I9 J6 G9 J9 J8 H10 H9 I10 G10 H11 K8 …`, 18手後黒番)、`#5` (`H8 G8 H9 F7 G9 E10 I9 F9 H7 H10 …`, 6手後黒番)。
