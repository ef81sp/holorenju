...
| bench（416 局）  | distinct @ply8 | @ply12 | @ply16 | 完全 distinct | 分岐 ply の中央値 |
| ---------------- | -------------- | ------ | ------ | ------------- | ----------------- |
| 2026-08-23T05-55 | 95             | 136    | 160    | 231           | 12                |
| 2026-08-23T08-14 | 103            | 133    | 163    | 232           | 14                |
| 2026-08-23T12-32 | 95             | 135    | 171    | 267           | 15                |

（ply は開局 3 手込みの moveHistory 先頭 n 手。レビュワー再計算で完全 distinct 266 を確認。）52 通りの（珠型 × 色）から 8 セットで 8 手目時点の進行が 100 通りしかない。半数近くが同一または終盤まで同一の棋譜で、独立サンプルとして数えられない。「個別 PR の +30/+22 が統合ベンチで再現しない」現象はこれで説明がつく。
...
- ペア LLR は正規近似 `LLR = N·(s1−s0)·(2s̄−s0−s1)/(2σ²)`（fastchess と同じ）。σ² は観測ペア得点の分散。**ガード**: ペア数 < 16 または σ² < 1e-4 のとき LLR=0（判定 continue）、CI は ±∞。
