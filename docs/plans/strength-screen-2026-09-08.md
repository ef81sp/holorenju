...
- 共通条件: `--fixed-nodes`（既定 1.2M・プローブ 6k）、`--jobs=7`。同時に走らせるベンチは 1 本。事前に `df`。
...
- **採用ゲート（候補 → 採用）**: ①時間モード `commit:bench --openings=v1 --jobs=5`（1,200 局 6 h）で **点推定 ≥ +10 かつ CI 下限 > −5（非劣性）**、
  ...
  ①は固定ノードでの検出力が時間モードと同等（§7.14）でも、製品は時間モードなので省略しない。時間モードで固定より弱く出た場合は不採用。
...
| 側      | 上限到達率 | 平均ノード/回 | プローブ比率 | 平均深さ | 1 手平均 |
| ------- | ---------- | ------------- | ------------ | -------- | -------- |
...
- 後続: 固定 N=1.2M の時間モード同等性（§7.11）は 6k で較正したもの。両モードとも同方向に変わったので大きくはずれない見込みだが、次の混合対局で再確認する。同方向もう 1 段（時間 15 ms / 固定 1.5k）は P1c として次のスクリーン。
