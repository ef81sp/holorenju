...
### 6.19 リリース前の総合ベンチ（2026-09-11、時間モード、A=main 6db1b2a / B=development dcf4888、v1 1,200 局、jobs=5、5.8 h、nohup）

- JSON `bench-results/commit-bench-2026-09-11T12-17-33-759Z.json`、abort 0、distinct 1200/1200
- **A=main 視点 −28.4 [−44.3, −12.7] ＝ development が +28.4**。pentanomial ll=116 ld=21 dd=378 wd=15 ww=70
- development 側: プローブ上限到達率 19.9%、1 手平均 5,035 ms、10 s 張り付き 22.3%、深さ 6.51（main 側は旧 wasm で統計フィールドなし）
- 内容: P1（プローブ上限半減 #163）＋ VCT 取りこぼし修正（#168）。#163 単体の +27.6 と整合し、#168 の境界判定（+6.4）を含めても総合で有意に正。**main リリース可**（ボス判断）。
