...
- 検出力（A≠B のときに符号を当てる）は §7.14 の陽性対照で時間モードと同等と確認済み（depth 5 を両モードで +35〜+41、CI 幅も同じ）。
...
| 項目                                                                   | 値                                                        | 根拠                                                                |
| ---------------------------------------------------------------------- | --------------------------------------------------------- | ------------------------------------------------------------------- |
| `PROBE_VCT_NODES_DETERMINISTIC`                                        | **6,000** → **3,000**（§7.15、2026-09-09）                | 時間モードの上限到達率 18.8% / 平均 2,009 ノードに一致（§7.8〜7.9） |
...
### 7.14 手順 6: 陽性対照（depth 7 vs depth 5、b9ad5bf、各 416 局、2026-09-07）

A=development（depth 7）vs B=`--max-depth-b=5` を、固定 1.2M（6k、jobs=7）と時間モード（jobs=5）で各 416 局。

|                            | 固定 1.2M                | 時間モード              |
| -------------------------- | ------------------------ | ----------------------- |
| ペア Elo（A 視点）         | **+41.1 [+13.9, +68.9]** | **+35.2 [+8.9, +61.9]** |
| pentanomial ll/ld/dd/wd/ww | 23 / 4 / 127 / 9 / 45    | 23 / 1 / 136 / 7 / 41   |
| avgDepth A / B             | 3.99 / 3.24              | 3.96 / 3.21             |
| 所要                       | 65 分                    | 87 分                   |
| abort                      | 0                        | 0                       |

- 両モードとも同方向・有意に検出。推定値も CI 幅（±27）もほぼ同じ。**検出力は時間モードと同等**で、固定ノードの利点は「負荷に依らず同一結果（再現性）」と「jobs を増やして短時間」にある。CI 幅は対局結果の分散が支配するので、固定ノードにしても狭くはならない（1,200 局で ±15 が要るのは同じ）。
- これで §4 の較正手順はすべて完了（手順 5 は §3 の実装時スモークと各スイープの全ペア同一結果で代替）。

### 7.15 追記（2026-09-09）: プローブ上限を 6k → 3k に変更

strength-screen-2026-09-08.md §6.3〜6.4 で時間モードの VCT プローブ上限を 50 → 25 ms に採用したのに伴い、
決定的モードの `PROBE_VCT_NODES_DETERMINISTIC` も到達率で対応する **3k** に変更（時間 25 ms 到達率 24.2% ↔ 固定 3k 25.4%）。
較正規則（§7.13「プローブ上限は到達率で合わせる」）は不変。N=1.2M の Elo 同等性（§7.11）は 6k 時点の値で、
3k では未再測（両モードが同方向に変わったので大きくずれない見込み。次の混合対局で確認）。

追記（2026-09-09 同日）: 混合対局（時間 25 ms vs 固定 1.2M/3k、v1 416 局 jobs=5）で **Elo 0 [−21.1, +21.1]**、到達率 24.5% vs 25.0%（strength-screen §6.9）。N=1.2M の同等性は 3k でも成立、既定値据え置き。
