...
手番をパス（何も打たない）して浅く探索し、それでも有利なら「この局面は有利すぎるので深く調べる必要がない」と判断して枝刈り。

```
パラメータ:
- NMP_MIN_DEPTH: 3（深度3以上で適用）
- NMP_REDUCTION: 2（探索深度を2浅くする）
```

**安全条件**:

- 相手に四（連続4個で片端以上開き）がない局面のみ適用
- 連続適用しない（allowNullMove フラグで制御）
- 勝ち/負け確定スコアでない局面のみ
...
浅い深度（depth 1-2）で、静的評価＋マージンが alpha を大幅に下回る手をスキップ。

```
パラメータ:
- depth 1: margin = 500（OPEN_THREE の半分程度）
- depth 2: margin = 1500（FOUR 程度）
```

**安全条件**:

- 四を作る手（戦術的な手）はスキップしない
- 最初の手（moveIndex = 0）はスキップしない
- 勝ち/負け確定スコアでない局面のみ

**難易度制限**: medium + hard で有効
...
1. **Opening Book**: 定石データベースの導入
2. **Pondering（先読み）**: プレイヤーの思考中にバックグラウンドで探索
